...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@gamma/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[gammaScrollbar]',
    exportAs: 'gammaScrollbar',
    standalone: true,
})
export class GammaScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_gammaScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() gammaScrollbar: boolean = true;
    @Input() gammaScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('gammaScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.gammaScrollbar = coerceBooleanProperty(
                changes.gammaScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.gammaScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('gammaScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.gammaScrollbarOptions.currentValue
            );

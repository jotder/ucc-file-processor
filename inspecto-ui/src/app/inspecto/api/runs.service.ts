...
export class RunsService {
  ...
  pause(name: string): Observable<{ pipeline: string; paused: boolean }> {
    return this.http.post<{ pipeline: string; paused: boolean }>(apiUrl(`/runs/${encodeURIComponent(name)}/pause`), {});
  }
  resume(name: string): Observable<{ pipeline: string; paused: boolean }> {
    return this.http.post<{ pipeline: string; paused: boolean }>(apiUrl(`/runs/${encodeURIComponent(name)}/resume`), {});
  }
  reprocess(name: string, batchId: string): Observable<Record<string, string>> {
    return this.http.post<Record<string, string>>(apiUrl(`/runs/${encodeURIComponent(name)}/reprocess`), { batchId });
  }
  ...
  lineage(name: string, batchId?: string): Observable<AuditRow[]> {
    return this.http.get<AuditRow[]>(apiUrl(`/runs/${encodeURIComponent(name)}/lineage`), { params: toParams({ batchId }) });
  }
  ...
  report(name: string, window?: ReportWindow): Observable<BatchAuditReport> {
    return this.http.get<BatchAuditReport>(apiUrl(`/runs/${encodeURIComponent(name)}/report`), { params: toParams({ ...window }) });
  }
}

...
public class IntegratedProcessor {
    ...
    public IntegratedProcessor(String walkRoot, String temp, String target, boolean dryRun)
            throws IOException {
    ...
    private boolean isWantedTar(Path file) {
        ...
        for (Path p : walkRoot.relativize(file))
    ...
    private void processArchive(Path src) {
        ...
        try {
            ...
            if (dryRun) {
                ...
                int count = TarUtil.extractTar(src, extractTarget);
                archivesProcessed.incrementAndGet();
    ...
    private void scanAndMove(Path dir) {
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
            ...
            System.err.println("[ERR] Scan error in " + dir);
        }
    }
    ...
    private void handleExtractedFile(Path file) {
        ...
        if (matcher.matches()) {
            ...
            try {
                ...
                System.err.println("[ERR] Move failed: " + filename);
            }
        }
    }

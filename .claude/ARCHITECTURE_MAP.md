...
- **`inspector/`** — `SourceProcessor` (poll-cycle entry: discover → stabilize → dedup → materialize → batch).
...
- **`event/`** (`EventLog`/`EventType`) · **`alert/`** (`AlertRule`/`AlertService`) · **`metrics/`**
  ...
  **`sql/`** (sandboxed SQL) · **`service/`** (`SourceService` host, `ControlApi` REST, `JobService`).
...
- **Main entry**: `com.gamma.inspector.SourceProcessor` (ETL) · `com.gamma.service.SourceService` (always-on

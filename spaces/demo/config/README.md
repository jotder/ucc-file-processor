...
| Job: backup + verify chain (System Maintenance P2) | `jobs/config_backup_job.toon` (nightly `backup` of `config/` → `data/backups`, SHA-256 manifest + `maintenance_backups` catalog Dataset) chained by `jobs/backup_verify_job.toon` (`on_signal: maintenance.backup.completed`) | `*_job.toon` | Workbench → Jobs |

...
import { GAMMA_MOCK_API_DEFAULT_DELAY } from '@gamma/lib/mock-api/mock-api.constants';
import { GammaMockApiService } from '@gamma/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(GAMMA_MOCK_API_DEFAULT_DELAY);
    const gammaMockApiService = inject(GammaMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = gammaMockApiService.findHandler(

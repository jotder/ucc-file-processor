...
>
> **Status: IN BUILD — P0 + P1(reachable) + P2 + P3a SHIPPED (2026-07-09; §17): registry + `run(JobContext)`
> bridge + Run Log (P0), signal ledger (P1a), on-signal triggers + `when` + composition (P1c), descriptor
> SPI (P2a), open string type ids + `ServiceLoader` (P2b), hot-deployable Job Packs (P2c), parameter resolver
> + `$`-context + fail-closed REJECTED (P3a — the reachable layers of P1b). Open questions resolved (§19).
> Next in P3: P3a-2 (trigger `args` + signal `bind` layers → `$signal.<field>`), P1d (Run Artifacts +
> `$upstream`), the `sql.template` Job Type (P3b), and the descriptor-driven authoring UI (P3c).**
...
| **P3 — Parameters, Templates & UI** (shipping as sub-commits) | Parameter resolver + `$`-context (P3a); trigger `args`/signal `bind` (P3a-2); Run Artifacts (P1d); `sql.template` Job Type (P3b); descriptor-driven authoring form (P3c, Workbench → Jobs, closes `ia-vocabulary-reorg` Phase D) | 🔨 **IN PROGRESS.** **P3a — parameter resolver ✅ SHIPPED (2026-07-09):** `ParameterResolver` (config → `deduce` `$`-context → `defaultValue`) wired into `runJob` for every Job Type; `$today`/`$day(-n)`/`$month(-n)`/`$now`/`$run.*`/`$job.last_success_time` (new durable `JobRunLedger.lastSuccessEnd`); missing-required ⇒ **REJECTED** pre-run (`job.run.rejected`); `JobContext.params()` exposes the resolved context. Reactor 1116/0/0/3 (+6 tests). **Remaining:** P3a-2 (args + `bind`/`$signal.<field>`), P1d (Run Artifacts + `$upstream`), P3b (`sql.template`), P3c (authoring UI). |

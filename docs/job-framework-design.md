...
| **P2 — Job Packs** (shipping as sub-commits) | `JobPackManager` (scan/watch/validate/quiesce), pack routes, signature flag, audit | 🔨 **IN PROGRESS.** **P2a — descriptor SPI ✅ SHIPPED (2026-07-09):** `ParameterDecl`/`ParamType`/`ArtifactDecl`/`JobTypeDescriptor`; `JobTypeProvider` now public with `descriptor()`; the four built-ins register real descriptors; `GET /jobs/types` + `GET /jobs/types/{id}` (R3 form-gen surface). Non-breaking. **Remaining: P2b** — open `Job.type()`/`JobConfig.type()` to string ids + deprecate the `JobType` enum + `ServiceLoader` classpath discovery (the one breaking refactor; unblocks module/pack-provided types being *referenced* by a `*_job.toon`). **P2c** — `JobPackManager` (watched `-Djobs.packs.dir`, isolated `URLClassLoader` per jar, `@JobTypeMeta` validate, quiesce/reload/unload, `GET /jobs/packs` + `POST /jobs/packs/rescan`, signature flag per §19.5, audit). |

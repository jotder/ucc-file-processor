...
>
> **Status: IN BUILD — P0 + P1(reachable) + P2 SHIPPED (2026-07-09; §17): registry + `run(JobContext)`
> bridge + Run Log (P0), signal ledger (P1a), on-signal triggers + `when` + composition (P1c), descriptor
> SPI (P2a), open string type ids + `ServiceLoader` (P2b), hot-deployable Job Packs (P2c). Open questions
> resolved (§19). Next: P3 — `sql.template` Job Type + descriptor-driven authoring UI, which carries the
> deferred, consumer-gated P1b (parameters) and P1d (Run Artifacts), since those go live with their first
> param-declaring / artifact-producing type.**

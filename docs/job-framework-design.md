...
| **P1 — Parameters, Signals, Artifacts** (shipping as 4 sub-commits) | `ParameterDecl`/resolver + `$`-context; `Signal` envelope + ledger over `EventLog` + `BatchEvent` adapters; `on_signal`/`bind`/`when`/`args` in `JobConfig`; `ArtifactRecorder` + `job_run_artifacts` + query routes; chain/storm protection | 🔨 **IN PROGRESS.** **P1a — Signals + ledger ✅ SHIPPED (2026-07-09):** `com.gamma.signal` (`Signal`/`Severity`/`SignalEmitter`/`Signals`) persists to the one ledger as `EventType.SIGNAL` (dotted type/severity/JSON payload in attrs, correlationId first-class); `JobContext.signals()`; framework lifecycle signals `job.run.started`/`completed`/`failed`; `GET /signals?type=&since=&correlationId=` (exact + `prefix.*` glob, correlation-filtered) via `SignalRoutes`; reactor 1339/0/0. **`BatchEvent` adapters deferred to P1c** (where `on_signal` consumes them). Remaining: **P1b** parameters+resolver · **P1c** triggers-v2 + composition + BatchEvent adapters + chain/storm · **P1d** Run Artifacts |

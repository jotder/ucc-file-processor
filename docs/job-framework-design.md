...
>
> **Status: IN BUILD — P0 + P1(reachable) + P2 + P3a + P1d SHIPPED (2026-07-09; §17): registry +
...
> parameter resolver + `$`-context + fail-closed REJECTED (P3a), Run Artifacts + `$upstream` (P1d). Open
> questions resolved (§19). Next in P3: P3a-2 (trigger `args` + signal `bind` → `$signal.<field>`), the
> `sql.template` Job Type (P3b — first real artifact producer), and the descriptor-driven authoring UI (P3c).**

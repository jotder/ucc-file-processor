# Stakeholder Document Set

> Compiled 2026-07-07 from the reconciled current-status docs. Each document targets one audience and
> stays deliberately short — it links to the authoritative deep docs rather than duplicating them.
> Vocabulary per [`../GLOSSARY.md`](../GLOSSARY.md).

| Audience | Read this | Then, if needed |
|---|---|---|
| **Executive / sponsor** | [`EXECUTIVE_BRIEF.md`](EXECUTIVE_BRIEF.md) | [`../roadmap/PRESENTATION_EXEC.md`](../roadmap/PRESENTATION_EXEC.md) (slides) |
| **Product owner / business analyst** | [`PRODUCT_CAPABILITIES.md`](PRODUCT_CAPABILITIES.md) | [`../REQUIREMENTS.md`](../REQUIREMENTS.md) (full MoSCoW) · [`../roadmap/ROADMAP.md`](../roadmap/ROADMAP.md) |
| **Solution architect / integrator** | [`TECHNICAL_ARCHITECTURE.md`](TECHNICAL_ARCHITECTURE.md) | the [OKF bundle](../okf/index.md) · [`../api/openapi-v1.json`](../api/openapi-v1.json) |
| **Operations / support** | [`OPERATIONS_GUIDE.md`](OPERATIONS_GUIDE.md) | [`../ADVANCED_GUIDE.md`](../ADVANCED_GUIDE.md) (the ops hub) |
| **End user (any Lens)** | [`../USER_GUIDE.md`](../USER_GUIDE.md) | — |
| **Mixed exec + technical briefing** | [`../roadmap/STAKEHOLDER_OVERVIEW.md`](../roadmap/STAKEHOLDER_OVERVIEW.md) | [`../roadmap/PRESENTATION.md`](../roadmap/PRESENTATION.md) |

**Keeping this set current:** these documents are snapshots over living sources. When status changes,
update the source doc first (REQUIREMENTS, ADVANCED_GUIDE, OKF), then refresh the affected stakeholder
doc and bump its date line.

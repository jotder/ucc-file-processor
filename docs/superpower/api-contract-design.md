...
| **W4** | **Query & results on DuckDB** | `POST /queries/{id}/run` with server-side `ParameterContext` (the R3 resolver contract, server-side) + Result Set descriptor + renderings; replaces the offline `runSpec` seam (= Widget-Library M2's `QuerySpec → DuckDB` item). | W1, W3 |
| **W5** | **Async & idempotency** | 202 + Run pattern on pipeline/job/import triggers; `GET /runs/{id}` progress; `Idempotency-Key` store; Run status ticks as Signals on the stream. | W1 |
| **W6** | **Security module** (`inspecto-security`) | `Authenticator` SPI impl (OIDC resource server: Nimbus + JWKS); permission resolver (claims → Roles → Capabilities → envelope verbs); HTTPS (`HttpsServer` + keystore); actor-attributed audit (retire `X-Actor`); UI OIDC Auth-Code+PKCE; WSO2 gateway definitions (routing/throttling/analytics per §8) + Keycloak realm blueprint. Edition assembly per `EDITIONS.md` (`-Pedition-standard`, `-Dauth.mode=oidc`). | W1 (chain), W2 (contracts to publish at the gateway) |

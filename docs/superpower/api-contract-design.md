...
| **W7** | **v1 migration — ✅ SHIPPED 2026-07-07 (UI; sunset deferred)** | The UI now speaks `/api/v1` end-to-end (plan: `w7-ui-v1-migration.md`). **Stated deviation: one global flip, not context-by-context** — W1's dispatch seam turned out to be fully path-generic (every route already served under `/api/v1`; verified by `ControlApiV1Test` + a backend survey), so per-context migration bought nothing; instead the whole change concentrates at three seams. (1) `apiUrl()` builds `/api/v1/…`; (2) a new first-position `v1Interceptor` unwraps success envelopes (`{data, metadata, diagnostics}` → `data`, shape-guarded via `isV1Envelope` so text/blob/304/legacy bodies pass through) — all 28 feature services keep their DTO signatures; TS contract types live in `inspecto/api/v1.ts`; (3) the mock layer envelopes at its response edge (`mock-api.interceptor.ts` + `v1SuccessBody`/`v1ErrorBody` porting `Envelope`/`ErrorCodes.defaultFor` — the 12 domain handlers stay raw-DTO, mirroring the backend seam). `apiErrorMessage` parses both error shapes; `spaceInterceptor` inserts the space id AFTER `/v1` (`/api/v1/spaces/{id}/…`). Jobs trigger consumers moved to the W5 async contract (202 + `{runId}`, "run started" toasts; mock aligned). **X-Actor needed no UI work** (never sent). **Follow-ons SHIPPED 2026-07-07:** (a) **legacy-alias usage logging** — a successful non-v1 call to a versioned route increments `inspecto_legacy_api_requests_total{route}` (infra probes health/ready/metrics excluded), the sunset signal a real deployment reads before removal (`ControlApi.recordLegacyUsage`; test `ControlApiV1Test.legacyUsageIsCountedForVersionedRoutesButNotV1OrInfra`); (b) **ADVANCED_GUIDE §10 regen** (v1-canonical framing + the sunset metric + the W5b run-trigger 202); (c) **X-Actor** is already Subject-preceded on Standard (W6) and its fallback stays for auth-free Personal — no code change. **Still deferred (correctly):** actual legacy-route *removal/rejection* — gated on the soak the new metric now measures. | W2–W6 |

| **W5b** | **Async pipeline triggers — ✅ SHIPPED 2026-07-07 (full-stack, mirrors W5 jobs)** | Pipelines were the last synchronous trigger; brought to the jobs async contract. **Backend:** `SourceService.triggerRunAsync(name)` mints a `runId`, registers a `RUNNING` `PipelineRun` in a bounded live-run LRU (mirrors `JobService`), and runs the existing sync `runPipeline` off the request thread on the **already-present `triggerWorkers` executor** — which was built precisely to run pipelines off the `ingestLock`-holding thread (the documented sync-bus+`ingestLock` deadlock), so this was infra-reuse, not an engine rewrite. `pipelineRunById(runId)` polls. `RunRoutes`: `POST /runs/{name}/trigger` returns **202 + `{runId,pipeline,status}` + `Location`** on v1; the **legacy unversioned route is byte-for-byte unchanged (200 + `RunResult`)** so all 6 existing sync-trigger tests stay green. **`GET /runs/runs/{runId}`** polls (single-segment, no route collision). **UI:** `runs.service.trigger` → `Observable<{runId}>`; sources + runs "run now" show a "run started" toast + reload (mirrors `jobs.component` exactly — no client polling); `demo.handler` mock returns 202 + adds the poll route. Tests: `ControlApiAsyncV1Test` (+3 pipeline cases: 202→poll→SUCCESS, legacy 200 unchanged, unknown 404); UI specs realigned. **Stated scope:** run status = the terminal `RunResult` counts on poll; the run-now toast drops the inline processed/failed counts (the list reload surfaces the outcome, same as jobs). | W5, W7 |

| **W8** | **Legacy sunset mechanism (API-5) — ✅ SHIPPED 2026-07-08 (core)** | The removal W7 deferred, made a per-deployment flag flip instead of a code change. Every successful legacy (non-v1) call to a versioned route now carries `Deprecation: @1783382400` (RFC 9745 — 2026-07-07, the day the SPA went fully `/api/v1`) + `Link: </api/v1>; rel="successor-version"`, and `Sunset: <HTTP-date>` (RFC 8594) once an operator signs a date via `-Dapi.legacy.sunset=YYYY-MM-DD`. **The flip:** `-Dapi.legacy.routes=off` answers **410 Gone** (legacy body shape, points at `/api/v1`) on the whole unversioned surface; infra probes (`/health`/`/ready`/`/metrics*`) stay exempt (no v1 semantics), v1 untouched, static/SPA serving untouched, and `inspecto_legacy_api_requests_total` keeps counting through the off-window so residual demand stays visible. **Runbook:** soak a deployment to 30 consecutive days at zero (product signs the criterion) → set `off` there → physical route-table deletion one release after every deployment runs `off` clean. Tests: `ControlApiLegacySunsetTest` (headers on legacy only, Sunset once signed, 410/200/200 split in off mode, metric still counted); `ControlApiV1Test`'s legacy byte-for-byte pin stays green (headers, not bodies). | W7 |

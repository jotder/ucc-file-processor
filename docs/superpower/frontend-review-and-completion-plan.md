...
| **1 — Builder: Workbench** | Connections + connection-workbench (247L) — ✅ shipped (ask-the-minimum two-step) · Sources · **Pipelines editor (752L — decompose: palette / canvas / inspector / state machine to lib)** · **parser-config dialog — ✅ shipped 2026-07-02** (first big SchemaForm conversion, all 9 parser kinds tiered; review sheet `docs/superpower/reviews/parser-config.md`) · node-config · Enrichment · Jobs + job-form (330L/222L) | heaviest UX debt, biggest forms, proves W1+W2 end-to-end |
...
1. Per pane × lens: which actions are visible to Business vs Builder vs Ops? (default: Business read-only everywhere except Requirements.)

...
| **2 — Builder: Studio** + W4 lens shell | **Dataset editor — ✅** (dup-guard; SchemaForm conversion evaluated + deferred, `reviews/dataset-editor.md`) · **Widget-options — ✅** (first Studio SchemaForm conversion; shared renderer gained an always-visible-optional tier, `reviews/widget-options.md`) · **Widgets/Explore — ✅** (`reviews/widgets-explore.md`) · **Dashboards editor — ✅** (`reviews/dashboard-editor.md`) · **Components/Registry — ✅** (already compliant, no changes needed, `reviews/registry.md`) · **Catalog + lineage graph — ✅** (empty-state fix + 2 new specs, `reviews/catalog-lineage.md`) · **Assist — ✅** (`reviews/assist.md`) — **all named panes reviewed 2026-07-02/03**; **W4 lens-switcher shell still NOT BUILT** | second authoring cluster; converts widget-options/dataset editors to SchemaForm; lens switcher (W4) is the one open item — it's what actually wires the confirmed Business-read-only Workbench gating |

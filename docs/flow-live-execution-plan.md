...
> `JobType.PIPELINE`, `PipelineJobRunner`, `PartitionSinkWriter`, `SourceStoreReader`, `JobService.build()` wiring,
> `SourceService.openFlowStore()`, the deletion fence (§7.5); a PIPELINE job is a first-class scheduled/chained/reported
...
Authored flows (created via the T19a UI, persisted by `com.gamma.pipeline.PipelineStore` under
`<write-root>/flows/*_flow.toon`) are today **CRUD-able and dry-runnable only** — nothing runs them
for real. `com.gamma.pipeline.exec.PipelineExecutor.execute(...)` exists (T12) and does the topological
...
- The "compile authored flow → `PipelineConfig` → run via the poll loop" shortcut **does not apply**:
  `PipelineCompiler.toConfigMap` only round-trips **lifted** graphs (typed `cfg` sub-records), not
  UI-authored ones (plain-map `cfg`), and the legacy pipeline shape can't express a rich multi-node
  transform DAG. Confirmed in [`PipelineCompiler.java`](../inspecto/src/main/java/com/gamma/pipeline/PipelineCompiler.java).
- The right path is to **drive `PipelineExecutor` directly**, hosted as a **`Job`** on the existing
...
**Run authored flows as a new job type, `JobType.PIPELINE`, implemented by a new `PipelineJobRunner implements
com.gamma.job.Job`, scheduled and audited by the existing `JobService`.** Reuse `PipelineExecutor` +
...
## 3. Run sequence (`PipelineJobRunner.run()` → `JobResult`)

```
1. open ephemeral DuckDB           DuckDbUtil.openConnection(tempDbFile)
2. seed each source_store as a view  for store in PipelineStores.consumed(graph):
     CREATE VIEW <store> AS SELECT * FROM <SqlViews.reader(format, "<dirs.database>/<store>/**/*.<ext>", hive=true)>
3. execute the flow                PipelineExecutor.execute(conn, graph, seedNodeId, seedView,
     ...
     - sourceFinalize = () -> {}        // no acquisition/markers for a job (no-op is valid, PipelineExecutorTest:77)
...
|---|---|---|
| `PipelineExecutor.execute(conn, g, seedNodeId, seedTable, batchId, coordinator, sinkWriter, sourceFinalize)` | **reuse** | [PipelineExecutor.java:72](../inspecto/src/main/java/com/gamma/pipeline/exec/PipelineExecutor.java) |
| `BranchCommitCoordinator` + `BranchCommitLog` (idempotent multi-branch commit) | **reuse** | [BranchCommitCoordinator.java:47](../inspecto/src/main/java/com/gamma/pipeline/exec/BranchCommitCoordinator.java) |
...
| `PipelineStore` (load authored `PipelineGraph` by id) | **reuse** | [PipelineStore.java](../inspecto/src/main/java/com/gamma/pipeline/PipelineStore.java) |
| `Job` interface + `JobConfig` + `JobResult` + `BatchEvent` | **reuse** | [Job.java](../inspecto/src/main/java/com/gamma/job/Job.java) |
| **`JobType.PIPELINE`** (new enum value + `from()` parse "pipeline") | **create** | [JobType.java:13](../inspecto/src/main/java/com/gamma/job/JobType.java) |
| **`PipelineJobRunner implements Job`** (orchestrates the §3 sequence) | **create** | new in `com.gamma.pipeline.exec` |
| **`PartitionSinkWriter`** (a `PipelineExecutor.SinkWriter` over `PartitionWriter`; resolves store→dir, format/partitions from the sink node `cfg`) | **create** | new in `com.gamma.pipeline.exec` |
| **`SourceStoreReader`** (helper: register a `source_store` as a DuckDB view via `SqlViews.reader`) | **create** | new in `com.gamma.pipeline.exec` |
| **`JobService.build()` case `PIPELINE -> new PipelineJobRunner(c, bus, flowsRoot, dataDir)`** | **edit** | [JobService.java:182](../inspecto/src/main/java/com/gamma/job/JobService.java) |

## 5. Config shape (`*_job.toon`)

```
name: nightly_rollup
type: pipeline             # new JobType
flow: events_rollup        # authored flow id (PipelineStore.get) — or flow_file: events_rollup_flow.toon
...
just params `PipelineJobRunner` reads. Scheduling lives in the **JobConfig** (consistent with all jobs), not
the PipelineGraph entry-node `trigger:` (that is for the pipeline poll loop). Activation = `JobConfig.enabled()`.

## 6. Phasing

- **Phase A (core run path) — ✅ DONE (2026-06-18).** `JobType.PIPELINE`, `PipelineJobRunner`, `PartitionSinkWriter`,
  `SourceStoreReader`; `JobService.build()` wiring (fail-closed without an authored-flow store) +
  `SourceService.openFlowStore()`. Manual run via the existing `POST /jobs/{name}/trigger` over a `type: pipeline`
  `*_job.toon`. Persistent + materialized sinks (unpartitioned single-file write when a sink declares no
  `partitions`). Full-recompute commit. Tested: `PipelineJobRunnerTest`(4) + `JobServiceTest`(+2). The dedicated
  `POST /pipelines/authored/{id}/run` was **not** built — the job-config path covers Phase A; revisit if ad-hoc
  (config-less) runs are wanted.
- **Phase B (scheduling + chaining) — ✅ DONE (2026-06-18).** No production change was needed — `JobService` arms
  cron and dispatches `on_pipeline` events type-agnostically, and `PipelineJobRunner` already publishes a chain
...
- **Phase C (semantics polish) — ✅ DONE (2026-06-18/19), one commit per slice.**
  - **Multi-`source_store`** (`93146d8`): `PipelineExecutor` gained a `Map<nodeId,seedView>` execute overload; a flow
  ...
  - **Incremental** (`dff1ac0`): opt-in `incremental_column` job param + `PipelineWatermarkStore` (file-based, keyed by
...
3. **`batchId` strategy** for idempotency: per-run timestamp (always recompute, simplest, MVP) vs a
   ...
   **[Phase A] `batch_id` job param when set (stable ⇒ idempotent replay skips committed branches via the
   ...
   + `PipelineWatermarkStore`: reads only rows past the stored watermark and appends (run-unique sink base), single-source.**
4. **Where data/audit dirs come from.** A job has no `PipelineConfig`; `PipelineJobRunner` needs the data
   ...
   **[Phase A RESOLVED] data root = `-Ddata.dir` (default `database`), overridable per-job via the `data_dir`
   ...
   both threaded `SourceService → JobService → PipelineJobRunner`.**
...
- **`PipelineJobRunnerTest`** (unit/integration, `--enable-native-access=ALL-UNNAMED`): seed a temp store dir
  with a small Parquet dataset; author a `source_store → transform.filter → sink.persistent` flow; run;
  assert the output Parquet exists with the expected rows (cross-check against `PipelineDryRun` counts on the
  same sample).
...
- **`JobService` integration:** a `type: pipeline` `*_job.toon` loads, `build()` produces a `PipelineJobRunner`,
...
~600–900 LOC across `PipelineJobRunner`, `PartitionSinkWriter`, `SourceStoreReader`, the `JobType`/`JobService`

...
| G6 | **SQL runs on an unsandboxed DuckDB connection** with full filesystem/extension access; no resident schema-loaded connection to reuse. | Agent-generated SQL needs a separate locked-down oracle (see v3-agent-mvp security). **✅ Resolved by M6 `com.gamma.sql` — `SqlGuard` (lexical allow-list) + `SqlSandbox` (register-then-seal: external access off, config locked) + `SqlOracle` (EXPLAIN/LIMIT-0) + `SqlViews` (extracted from `EnrichmentEngine`) (v3.6.0).** |
...
| **Assist platform + `explain-entity`** (`ModelProvider`/`ModelRouter`/`AssistProfile` seam over LangChain4j+Ollama, `SkillRegistry`, `ExplainEntitySkill`, `AiDescriptionProvider`, `AssistRequest`/`AssistResult` DTOs + `AssistAgent.assist` + `POST /assist/{intent}`) | core seam in `com.gamma.assist`/`ControlApi`; all AI in `com.gamma.agent`(`.model`/`.skill`/`.catalog`) | V-1/V-3/V-4 | **M3 / v3.3.0 ✅ shipped (local-only; lean core stays 0-AI)** · **4.0:** the AI module is migrated onto the reusable `agent-kernel` (`Skill`→`Capability`, `SkillRegistry`→`CapabilityRegistry`, model/`RepairLoop`/`DocRetriever` consumed from the kernel); the lean core stays kernel-free (CI-guarded); `AssistResult.confidence` reshaped **`String`→`double`** (the one sanctioned 4.0 break) |
| **`nl-to-schedule` (draft-only) + the generate→validate→repair seam** (`NlToScheduleSkill`, `CronDescriber`, `RepairLoop`; reuses the core cron oracle `CronExpression` + `JobConfig.fromMap` + `ConfigSpecs.job()`; additive `AssistResult.data` structured payload) | one additive core field on `AssistResult`; all logic in `com.gamma.agent.skill` | V-5/V-8/V-9 | **M4 / v3.4.0 ✅ shipped (draft-only — `applyVia` null; lean core stays 0-AI, 0 new deps)** |
| **`suggest-config` (draft-only) + hard-fail config safety validator** (`SuggestConfigSkill`; new core `com.gamma.config.safety` = `SafetyPolicy` + `ConfigSafetyValidator`: path jail / numeric bounds / output allow-list; opt-in on `POST /validate`; reuses `RepairLoop` + `ConfigSpecs.forType` + `AssistResult.data`) | core safety validator in `com.gamma.config.safety` + `ControlApi` opt-in; skill in `com.gamma.agent.skill` | V-9 / R6 | **M5 / v3.5.0 ✅ shipped (draft-only; security guardrail R6; core zero-dep, 0 AI; production config-load path untouched)** |
| **`kpi-to-sql` (the hero, draft-only) + locked-down SQL sandbox** (`KpiToSqlSkill`, LARGE tier; new core `com.gamma.sql` = `SqlGuard` + `SqlSandbox`/`SqlSandboxPolicy` + `SqlOracle` + `SqlViews` extracted from `EnrichmentEngine`; reuses `RepairLoop` + `AssistResult.data`; grounds on the M1/M2 catalog + KPI catalog; confirm-first; opt-in sample rows) | core sandbox in `com.gamma.sql` (zero new deps — DuckDB already core); skill in `com.gamma.agent.skill` | G6 / V-9 | **M6 / v3.6.0 ✅ shipped (draft-only; closes G4/G6; core 0 AI, 0 new deps)** |
| **Security hardening** (scoped tokens, constant-time compare, no open default) | `ControlApi` | G7 | MVP |
| **`diagnose-and-alert` (event-driven) + failure-event seam** (`BatchEvent` enriched with `error`/`offendingFile`/`errorRows` + back-compat ctor; agent `com.gamma.agent.diagnose` = `FailureReactor` (bounded-queue daemon virtual-thread hand-off, never blocks ingest) + `HeuristicDiagnoser` + `ModelDiagnoser` + `DiagnosisStore`; `DiagnoseAndAlertSkill` NL→alert-rule draft over `AlertRuleValidator`; core `Diagnosis` DTO + additive `AssistAgent.recentDiagnoses` SPI + `GET /assist/diagnoses`) | `BatchEvent`/`BatchAuditWriter` + `Diagnosis`/`AssistAgent`/`ControlApi` in core; all AI in `com.gamma.agent.diagnose`/`.skill` | C1 alerts / V-10 | **M7 / v3.7.0 ✅ shipped (event-driven; abstain-safe heuristic + model uplift; draft-only; core 0 AI, 0 new deps; MVP skill catalog complete)** |
| **`report-sql` + `report-narrative` (operational reporting, draft-only)** (`ReportSqlSkill`, MEDIUM, over the operational ledgers via the M6 sandbox's additive in-memory `SqlOracle.TableData` mode + `OperationalTables` name→fixed-schema resolver; `ReportNarrativeSkill`, SMALL, strictly-extractive with the deterministic `NarrativeGuard`, abstain-safe template fallback; reuses `RepairLoop`/`AssistResult.data`; new read-only `SourceService.configSource()` handle on `AssistContext`) | additive `SqlOracle.TableData` + `SourceService.configSource()` in core; skills in `com.gamma.agent.skill` | B2 / V-9 | **M8 / v3.8.0 ✅ shipped (draft-only; sandbox boundary holds for operational data; core 0 AI, 0 new deps)** |

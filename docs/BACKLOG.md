...
| System Maintenance module | MoSCoW adapted to Inspecto (MNT-1…16) + 3 phases. **P1 SHIPPED 2026-07-12** (dry-run `?dryRun=true` framework-wide, `runlog_prune`, cleanup policy knobs incl. archive-instead-of-delete, `storage_report`, `scheduler_audit` — all on the built-in `maintenance` job type; demo job + descriptor-driven forms). Open: P2 backup+verify+restore+`metadata_validate`, P3 nightly composed pipeline + Maintenance menu/Overview pane + `/health/details` (IA in plan §5: new top-level "System Maintenance" nav group, sibling of Operations) | **P1 done; P2–P3 open** | `superpower/system-maintenance-plan.md` |

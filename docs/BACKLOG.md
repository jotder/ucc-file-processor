...
| **Notifications** | Email channel impl + delivery-status webhooks · digest batching · time-based retention sweep · GeoIP · auth-gated per-user prefs/security triggers *(2026-07-18: **channels admin CRUD** SHIPPED backend — `GET/POST /notifications/channels` + `PUT/DELETE /notifications/channels/{id}` persist a `channel` ComponentStore kind (`ChannelConfig`); 503/422/409/404 gates, per-space, `canAuthorWorkbench`. **Admin config only** — live delivery still resolves channels from `notify.*` JVM flags via the `NotificationChannel` SPI; wiring persisted destinations into dispatch is the open follow-up)* | `archived-documents/plans-archive/notification-system-and-audit-trail-plan.md` |

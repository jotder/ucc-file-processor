...
| **Signal / Decision networks** | Raw `/signals` endpoint + `SignalsService` (backend Signal contract) · Expectation/Alert-Rule ComponentKind promotion · LLM-backed Assist proposal *(2026-07-18 SHIPPED: real condition-tree evaluator `ConditionTree` — decision-rule `simulate` evaluates `when` over `sampleRows` with query-eval.ts parity. Open follow-ons: routing consequences `route`/`tag`/`quarantine`/`drop` still need live-pipeline-execution wiring to take effect at apply-time; the **mock** `decision-rules.handler.ts` simulate still returns canned `demoMatched`/`demoTotal` (should port `evaluateRows` over `sampleRows` for offline parity); and the **UI** Simulate action must send a `sampleRows` sample for either backend to be useful)* | `archived-documents/plans-archive/signal-network-plan.md` · `decision-network-plan.md` · `okf/backend/control-plane/decision-rules.md` |

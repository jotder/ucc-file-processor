...
| AGT-5 · kpi_report_builder | P2 authoring tool: compose `MeasureCompiler` measures + `kpi`/`chart` widgets into a persisted DRAFT `dashboard` | **OPEN — product-gated.** `dashboard`/`widget` kinds persist, but `dashboard` has **no `ConfigSpec`** (so drafts can't run the validator-repair loop) and the tile shape needs the dashboard-owner's sign-off. Decide shape before building. |
...
| **Incident/Case: enforce ≥1 linked entity at creation** (product decision 2026-07-22). Today `POST /objects` is title-only + a separate `POST /objects/{id}/links` (create-then-link). Flipping to link-at-create spans: `ObjectRoutes.createObject` (accept a `links[]`, 400 if empty, link after `open()`), the UI `object-create.dialog` (collect ≥1 link), the mock create handler, and ~9 `ControlApi*Test` files that create link-less objects. Route-level only — the auto-creation paths (`DecisionRoutes`/`ExpectationRoutes`/`AlertService`/`ReconRunJob`/`EventObjectBridge`) call `ObjectService.open` directly and stay unaffected. | Open (bounded) | `inspecto/.../control/ObjectRoutes.java:258` · `okf/frontend/features/objects.md` |
...
| FEATURE_INVENTORY residual verification (was recs #9) — re-check the doc's remaining claims against current code and update `FEATURE_INVENTORY.md` directly (most rows — `record_split`, example files — already shipped, see the 2026-07-20 note below). Not a product question, a verification chore. | Open (chore) | `FEATURE_INVENTORY.md` §6 |

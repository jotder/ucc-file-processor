...
| System Maintenance module | MoSCoW adapted to Inspecto (MNT-1…16) + 3 phases. **P1 SHIPPED 2026-07-12** (dry-run `?dryRun=true` framework-wide, `runlog_prune`, cleanup policy knobs, `storage_report`, `scheduler_audit`). **P2 SHIPPED 2026-07-12** (`backup`/`backup_verify`/`restore` with SHA-256 sidecar manifests + `maintenance_backups` catalog Dataset, `metadata_validate`, `db_maintenance` over all DuckDB stores, `min_keep`; runbook `docs/ops/backup-restore-runbook.md`; demo backup→verify signal chain). Open: P3 nightly composed pipeline template + Maintenance menu/Overview pane + `/health/details` (IA in plan §5: new top-level "System Maintenance" nav group, sibling of Operations) | **P1+P2 done; P3 open** | `superpower/system-maintenance-plan.md` |

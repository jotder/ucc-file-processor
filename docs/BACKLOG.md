...
| **Branding / misc backend** | Real `GET\|PUT /settings/branding` (mock-only persistence) · report-run artifact **content** download route (metadata ships) · Enrichment routes read-only stub (documented) · db-browser: promote `DbJobRunStore.detectPostgres` to a shared helper + `DbAcquisitionLedger` Postgres round-trip test · legacy-alias usage logging for the W7 sunset · `SpaceManager.delete` doesn't `forget(spaceId)` on ConnectionRegistry/StabilityGate (minor) | gap sweeps · `archived-documents/plans-archive/db-browser-design.md` · `w7-ui-v1-migration.md` |

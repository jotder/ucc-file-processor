...
| **Pipeline graph** | T15 adaptive back-pressure config · T19 topology-editor wiring into live executor (CRUD backend done) · T32 config-less ad-hoc run route + `sink.view` `derived_sql` · mock-only: run-to-here `POST …/run`, `/config/icon-map`, `/asn1/modules` *(2026-07-18: T16/T17 checklist corrected — both were already shipped/closed, see the design doc §14 for the as-built; T17's live last-run overlay closed this pass by wiring the existing-but-unused `/provenance` endpoints into the editor canvas + inspector)* | `okf/backend/pipeline-graph/pipeline-graph-design.md` §14 · `archived-documents/plans-archive/flow-live-execution-plan.md` |

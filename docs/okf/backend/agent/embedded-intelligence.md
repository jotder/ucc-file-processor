...
## P2 authoring tier (shipped — 5 of 5 tools)
...
are `FunctionTool`s on the belt (now **14** non-mutating of 21 total), `mutating=false` — they persist nothing.
...
- **`component_draft`** (the validator repair loop) — validates a proposed draft against the *same*
  ...
  the spec-backed set: `pipeline|enrichment|job|schema|expectation|alert-rule|widget|dashboard`
...
- **`kpi_report_builder`** (measures → a KPI report) — composes a dataset ref + a list of measures
  (`{agg,field}`, agg ∈ `count|countDistinct|sum|avg|min|max`) into DRAFT `widget` components (one `kpi`
  per measure, or a single grouped `bar` chart when `groupBy` is given) **and** a DRAFT `dashboard` that
  tiles them — the model never writes widget/dashboard JSON. `controls` are built server-side to the exact
  persisted shape (`value` for kpi; `x`/`y`/`series` for charts, per real widget TOON); each draft is
  validated against its `ConfigSpec` (the new `ConfigSpecs.widget()`/`dashboard()`, mirroring
  `widget-types.ts`/`dashboard-types.ts`), and the bundle (`{kind:dashboard, clean, findings, draft,
  widgets:[{id,draft}]}`) is promoted via `component_apply` (widgets first, then the dashboard). Uses
  ad-hoc `{agg,field}` measures — there is no server-side `NamedMeasure` resolver (deferred). Shipped
  2026-07-22 with operator sign-off on the bundle + measure-ref shape; the P2 blocker was the missing
  `dashboard`/`widget` `ConfigSpec` (now authored, which also lets `component_draft` validate those kinds).
  `Capability.AUTHOR_PIPELINE`.

...
tags: [architecture, feature-based, standalone, signals, layering, shared-libraries]
...
* `geo/` — the MapLibre host + `GeoSource`/`GeoQuery` for the Geo Map studio.
* `investigation/` — `SavedViewStore`, the detail dialog, and `uniqueNameValidator` (shared by Link Analysis + Geo Map).

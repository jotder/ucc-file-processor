...
description: Reusable grammar/schema/transform/sink/alert components, the /components + /pipelines routes, preview and safe-delete.
...
* **Safe delete**: `PipelineReferences` scans every flow's nodes for `use:` references; `ComponentRoutes` returns
...
* `/pipelines…` — `PipelineRoutes` (`inspecto/src/main/java/com/gamma/control/PipelineRoutes.java`): `GET /pipelines`
  ...
  store-joined pipeline+job topology), and `/pipelines/authored/*` CRUD + `dry-run` for authored flows. The store
  superimposition (`PipelineStores.superimpose`) joins consumer `source_store` names to producer `store` names —

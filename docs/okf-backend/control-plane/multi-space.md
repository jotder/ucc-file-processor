...
current/default space. Space CRUD (server-global, un-prefixed): `GET/POST /spaces`,
`DELETE /spaces/{id}?purge=`, and `GET /spaces/_meta → {multiSpace}` (the UI capability probe — never infer
from the space-list length). The UI side is the spaces feature + the global space interceptor (see the
inspecto-ui bundle, `inspecto-ui/docs/okf/`).

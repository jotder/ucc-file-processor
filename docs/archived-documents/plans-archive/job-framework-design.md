...
>
> **Status: SHIPPED — P0 + P1(reachable) + P2 + P3 COMPLETE (2026-07-09; §17): registry +
> `run(JobContext)` bridge + Run Log (P0), signal ledger (P1a), on-signal triggers + `when` + composition
> (P1c), descriptor SPI (P2a), open string type ids + `ServiceLoader` (P2b), hot-deployable Job Packs (P2c),
> parameter resolver + `$`-context + fail-closed REJECTED (P3a), trigger `args` + signal `bind` →
> `$signal.<field>` completing the §7.2 ladder (P3a-2), Run Artifacts + `$upstream` (P1d), the
> `sql.template` Job Type — first real artifact producer, config-aware parameters scanned from its SQL (P3b),
> and the descriptor-driven jobs authoring UI (P3c). Open questions resolved (§19).**
...
> **As-built note (P1d, 2026-07-09).** `ArtifactRecorder`/`ResultSetMeta`/`RunArtifact` ship; `ctx.artifacts()`
> persists dataset/file artifacts as per-run JSONL under `<auditDir>/artifacts/<runId>.jsonl` (`RunArtifactStore`
> mirrors `RunLogStore` — always-on, independent of the optional DuckDB `-Djobs.backend`). Read back by
> `GET /jobs/{name}/runs/{runId}/artifacts` and `GET /jobs/{name}/artifacts/latest` (latest successful run via
> durable `JobRunLedger.lastSuccessRunId`), and by the `$upstream(<job>).artifact(<name>).<attr>` resolver token.
> **Deferred:** the default `ViewDefinition` auto-registration bullet above (the `ViewStore` bridge) — recording
> the metadata is P1d; making the output bindable via `/views` lands with its first dataset producer (`sql.template`,
> P3b). The persistence grain is JSONL, not the DuckDB `job_run_artifacts` table the header sketches — same
> query surface, chosen to match the always-on `RunLogStore` idiom.
...
> **As-built note (P2c, 2026-07-09).** Packs are loaded from a **private staged copy** of each jar, not the
> watched-dir jar itself, so the deployment jar stays unlocked and an admin can delete/replace it even on
> Windows (a plain `URLClassLoader` would otherwise pin the file and make removal — hence unload — impossible).
> `rescan()` is the idempotent reconciler (new ⇒ load, gone ⇒ unload, changed-by-hash ⇒ reload) run at
> construction (before Jobs are built), on each settled `WatchService` batch, and via `POST /jobs/packs/rescan`.
> **Deferred (§12.2 refinements, not yet built):** deep *quiesce* of an in-flight Run onto its old classloader
...
`parameters(config)` reports `event_date` (required); the UI form shows exactly that contract plus the
static config keys (`sql`, `sink_dataset`, `sources`).

> **As built (P3b, 2026-07-09):** `SqlParamScanner.scan` types every scanned token `STRING`/`required`
> with **no `deduce`** — so the author supplies `event_date` explicitly (config `params:`, a trigger arg,
> or a signal `bind:`), or the Run is `REJECTED` fail-closed. Type/`deduce` **inference by name** (e.g.
> `*_date` → `DATE` + `deduce: $day(-1)`) is a deliberate follow-on, not built. Other deviations from the
> sketch above: `dataDir` is **constructor-injected** (the built-in convention — no `ctx.services()`
> façade was added for a single consumer); the output is a **single stage-and-swap snapshot** (`COPY … TO
> file`, mirroring `MaterializeTask`), not `PartitionWriter` Hive-partitioning — the example's result has
> no partition columns. Parameters substitute as single-quote-escaped SQL **literals** (injection-safe;
> DuckDB casts in a predicate). The sink is recorded as a `RunArtifact` (discoverable + `$upstream`-
> chainable); registering it as a queryable `dataset` component / `ViewStore` entry (§10) stays deferred.
...
| **P1 — Parameters, Signals, Artifacts** (shipping as 4 sub-commits) | `ParameterDecl`/resolver + `$`-context; `Signal` envelope + ledger over `EventLog` + `BatchEvent` adapters; `on_signal`/`bind`/`when`/`args` in `JobConfig`; `ArtifactRecorder` + `job_run_artifacts` + query routes; chain/storm protection | 🔨 **IN PROGRESS.** **P1a — Signals + ledger ✅ SHIPPED (2026-07-09):** `com.gamma.signal` (`Signal`/`Severity`/`SignalEmitter`/`Signals`) persists to the one ledger as `EventType.SIGNAL` (dotted type/severity/JSON payload in attrs, correlationId first-class); `JobContext.signals()`; framework lifecycle signals `job.run.started`/`completed`/`failed`; `GET /signals?type=&since=&correlationId=` (exact + `prefix.*` glob, correlation-filtered) via `SignalRoutes`. **P1c — Triggers-v2 + composition ✅ SHIPPED (2026-07-09):** `on_signal` + `when` keys on `JobConfig` (back-compat ctor keeps the 7-arg call sites); `JobService` subscribes its space's `EventLog` for on-signal dispatch (self-loop guard, `WhenGuard` guard → `SKIPPED`, per-job `TriggerCoalescer` storm-fold, correlationId + `chainDepth` inheritance with a `maxChainDepth` cut → `job.chain.cut`); `BatchEvent`→`pipeline.commit` signal mirror (coexists with the unchanged `on_pipeline` path). **The reachable P1 (signals + composition) is COMPLETE.** **P1b — parameters (reachable layers) ✅ SHIPPED (2026-07-09, as P3a):** `ParameterResolver` wired into `JobService.runJob` for every Job Type — resolves each declared `ParameterDecl` across config → `deduce` (`$`-context: `$today`/`$day(-n)`/`$month(-n)`/`$now`/`$run.id|fire_time|actor`/`$job.last_success_time` via new durable `JobRunLedger.lastSuccessEnd`) → `defaultValue`; a missing `required` parameter fails the Run **REJECTED** before user code (`job.run.rejected` signal + Run Log). `JobContext.params()` exposes the resolved Parameter Context (distinct from raw `config()`). **P3a-2 ✅ SHIPPED (2026-07-09):** the **args** (layer 1) + **signal `bind`** (layer 2 → `$signal.<field>`) layers now complete the §7.2 ladder — `JobConfig` parses nested `args:`/`bind:`; `JobService` threads a per-firing `Firing(args, signalPayload)` through `submitRun`→`runJob` (manual `POST` body `params:{}` + static config `args:` merge as layer 1, explicit wins; on-signal dispatch supplies the firing payload against which `bind:` resolves `$signal.<field>`). **P1d** (`ArtifactRecorder`/`ResultSetMeta`/`job_run_artifacts` + `$upstream`) lands with its first artifact-producing Job Type (`sql.template`, P3b), where `$upstream`↔artifacts become live together. |
| **P2 — Job Packs** (shipping as sub-commits) | `JobPackManager` (scan/watch/validate/quiesce), pack routes, signature flag, audit | 🔨 **IN PROGRESS.** **P2a — descriptor SPI ✅ SHIPPED (2026-07-09):** `ParameterDecl`/`ParamType`/`ArtifactDecl`/`JobTypeDescriptor`; `JobTypeProvider` now public with `descriptor()`; the four built-ins register real descriptors; `GET /jobs/types` + `GET /jobs/types/{id}` (R3 form-gen surface). Non-breaking. **P2b — open type + ServiceLoader ✅ SHIPPED (2026-07-09, `feat!` breaking→5.0):** `Job.type()`/`JobConfig.type()` now return the open String registry id (was the `JobType` enum, now `@Deprecated`, kept for source compat + built-in id constants + two back-compat ctors); `/jobs` + run records report the lowercased id; an unknown id parses (open registry) and fails closed at build; `JobTypeRegistry` also loads classpath `JobTypeProvider`s via `ServiceLoader` (id collision with a built-in rejected fail-closed); `ConfigSpecs` `job.type` enum→open STRING. **P2c — Job Packs ✅ SHIPPED (2026-07-09):** `JobPackManager` (off unless `-Djobs.packs.dir` is set) scans at construction + watches with `WatchService` (settle-delayed); each jar loads via its own parent-first `URLClassLoader` from a **private staged copy** (watched jar stays unlocked ⇒ delete/replace works on Windows), 2-arg `ServiceLoader.load(JobTypeProvider.class, packLoader)` filtered to the pack's own loader, `@JobTypeMeta` id cross-checked vs descriptor; `rescan()` is the idempotent reconciler (new⇒load / gone⇒unload / changed-by-hash⇒reload) also on `POST /jobs/packs/rescan`; a whole pack is rejected atomically on collision/validation failure; `JobTypeRegistry` gains owner-tracked `register`/`deregister` (built-ins never displaced); `GET /jobs/packs` inventory; `job.pack.loaded`/`unloaded`/`rejected` audit signals; `-Djobs.packs.requireSignature` (default-off) rejects unsigned entries, trust-anchor match against `-Djobs.packs.trustStore` left as the SEC-7 gate. **Deferred (§12.2):** deep in-flight-Run quiesce across reload + authored-Job `unavailable`/`REJECTED` on unload — land with P3's run-lifecycle work. Reactor 1110/0/0/3 (+3 isolated-classloader pack tests). **P2 COMPLETE.** |
| **P3 — Parameters, Templates & UI** (shipping as sub-commits) | Parameter resolver + `$`-context (P3a); trigger `args`/signal `bind` (P3a-2); Run Artifacts (P1d); `sql.template` Job Type (P3b); descriptor-driven authoring form (P3c, Workbench → Jobs, closes `ia-vocabulary-reorg` Phase D) | 🔨 **IN PROGRESS.** **P3a — parameter resolver ✅ SHIPPED (2026-07-09):** `ParameterResolver` (config → `deduce` `$`-context → `defaultValue`) wired into `runJob` for every Job Type; `$today`/`$day(-n)`/`$month(-n)`/`$now`/`$run.*`/`$job.last_success_time` (new durable `JobRunLedger.lastSuccessEnd`); missing-required ⇒ **REJECTED** pre-run (`job.run.rejected`); `JobContext.params()` exposes the resolved context. Reactor 1116/0/0/3 (+6 tests). **P1d — Run Artifacts ✅ SHIPPED (2026-07-09):** `ResultSetMeta`(`Column`+`Role`)/`ArtifactRecorder`/`RunArtifact`; `JobContext.artifacts()` records dataset/file artifacts to per-run JSONL (`RunArtifactStore` under `<auditDir>/artifacts/`, mirrors `RunLogStore` so it's always-on regardless of `-Djobs.backend`); `GET /jobs/{name}/runs/{runId}/artifacts` + `GET /jobs/{name}/artifacts/latest` (latest successful run via new durable `JobRunLedger.lastSuccessRunId`); the `$upstream(<job>).artifact(<name>).<attr>` token (`ref`/`rows`/`bytes`/`watermark`/`time_range`) resolves in `ParameterResolver`. **Deferred:** auto-registering a queryable `ViewDefinition` for a dataset artifact (the `ViewStore` bridge, §10). Reactor 1119/0/0/3 (+3 tests). **P3a-2 — trigger args + signal bind ✅ SHIPPED (2026-07-09):** the §7.2 ladder is complete — `resolve` now spans trigger args → signal `bind:` (`$signal.<field>` against the firing payload) → config → deduce → default; `JobConfig` parses `args:`/`bind:`; `POST /jobs/{name}/trigger` accepts a `{"params":{}}` body of explicit args. Reactor 1123/0/0/3 (+4 tests). **P3b — `sql.template` Job Type ✅ SHIPPED (2026-07-09):** the first real Run Artifact producer — `SqlTemplateJobType`/`SqlTemplateJob` + `SqlParamScanner`; the authored SQL's `$name` tokens **are** its runtime parameter contract (config-aware `JobTypeProvider.parameters(JobConfig)`, the R3 "queried through interfaces" seam, now routed through `JobService.runJob`); runs over source Datasets registered as DuckDB views (`SourceStoreReader`), materializes the result as a stage-and-atomic-swap snapshot Parquet Dataset (mirrors `MaterializeTask`), and records `ctx.artifacts().dataset(...)` + emits `job.dataset.produced`. As-built deviations from the sketch below: constructor-injected `dataDir` (built-in convention) rather than a `JobContext.services()` façade (avoids a single-use abstraction); single-snapshot COPY rather than `PartitionWriter` Hive-partitioning (the example result carries no partition columns). Params substitute as single-quote-escaped SQL literals (injection-safe; DuckDB casts in predicates). **Deferred:** auto-registering the sink as a queryable `dataset` component / `ViewStore` entry (the §10 bridge) — the RunArtifact already makes it discoverable + `$upstream`-chainable. Reactor 1129/0/0/3 (+6 tests). **P3c — descriptor-driven authoring UI ✅ SHIPPED (2026-07-09):** the `inspecto-ui` jobs form (`job-form.dialog`) is now descriptor-driven — `JobsService.types()`/`describeType(id)` hit `GET /jobs/types[/{id}]`; selecting a Job Type fetches its `parameters` and renders them as a typed `<inspecto-schema-form>` (`job-parameter-specs.ts` maps `ParameterDecl`→`AttributeSpec`; `sql`→multiline, `deduce`→placeholder+help, `required`→tier), so a new type (incl. a Pack's) surfaces its contract with no UI change; the free key/value editor is kept for undeclared extras. Mock handler serves the descriptors offline (binding constraint). Closes `ia-vocabulary-reorg` Phase D. UI DoD green (lint:tokens, 16 targeted specs, prod build, live preview). **P3 COMPLETE — the Job Framework is fully shipped.** |

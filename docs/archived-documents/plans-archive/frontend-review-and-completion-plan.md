...
| C3 | **Decision Rule builder** — business routing rules + consequences, surfaced as first-class (today buried in `transform.route`) — **✅ SHIPPED 2026-07-04** (`/decision-rules` Workbench pane; ConditionGroup when-clause + stacked consequences (route/tag/quarantine/drop); dry-run simulate, no incidents; MOCK_STORE_KEY→v6; `reviews/decision-rules.md`) | Builder | C2 patterns | **P2** |
| C6 | **Scheduled reports** — Dashboard/Report on a Trigger → export (PNG exists; add PDF/CSV mock + schedule UI) — **✅ SHIPPED 2026-07-04** (extends KPI & Reports, no new entity: a schedule IS a `type:'report'` Job; real CSV export + PDF/PNG placeholders; REPORT_EXPORTED fan-out; `reviews/scheduled-reports.md`) | Builder/Business | W1, Jobs review | **P2** |
| C5 | **Link Analysis studio** — Entity/Link Graph Visualization Type; Widget binds a Dataset via mapping wiring (per GLOSSARY §11 P3 design); pivot/expand UI inside Cases — **✅ SHIPPED (MVP) 2026-07-04** (`/studio/link-analysis`: GraphSource seam over all 4 planes + entity projection + path/explain/centrality/communities + saved `link-analysis-view` Components + PNG/JSON export; MoSCoW release phasing in `link-analysis-studio-plan.md` §6; `reviews/link-analysis-studio.md`) | Builder/Business | W3 graph lib, Studio wave | **P2** (keystone of the link-analysis template + xDR story) |

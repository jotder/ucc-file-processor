...
class ConservationCheckTest {
    ...
    private static FlowGraph filterFlow() {
        return new FlowGraph("F", true,
                List.of(FlowNode.of("src", "parser"),
                        FlowNode.of("flt", "transform.filter", Map.of("where", "amt >= 100")),
                        FlowNode.of("out", "sink.persistent", Map.of("store", "o"))),
    ...
    void splitNodeIsNotFlaggedEvenThoughItAmplifies() {
        ...
        FlowGraph g = new FlowGraph("S", true,
                List.of(FlowNode.of("src", "parser"),
                        FlowNode.of("sp", "transform.split", Map.of("column", "tags")),
                        FlowNode.of("out", "sink.persistent", Map.of("store", "o"))),
                List.of(FlowEdge.data("src", "sp"), FlowEdge.data("sp", "out")));
    ...
    void caseRouteConservesButCloneRouteIsSkipped() {
        ...
        FlowGraph caseG = routeFlow("case");
        ...
        FlowGraph cloneG = routeFlow("clone");
    ...
    private static FlowGraph routeFlow(String mode) {
        return new FlowGraph("R", true,
                List.of(FlowNode.of("src", "parser"),
                        FlowNode.of("r", "transform.route", Map.of("mode", mode)),
                        FlowNode.of("hi", "sink.persistent", Map.of("store", "hi")),
                        FlowNode.of("lo", "sink.persistent", Map.of("store", "lo"))),
                List.of(FlowEdge.data("src", "r"),
                        new FlowEdge("r", FlowRel.route("hi"), "hi"),
                        new FlowEdge("r", FlowRel.route("lo"), "lo")));
    }
}

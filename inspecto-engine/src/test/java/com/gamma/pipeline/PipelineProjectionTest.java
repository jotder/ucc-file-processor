...
/**
 * {@link PipelineProjection}: the read-only IR→JSON projection that feeds the UI palette + G6 renderer
...
class PipelineProjectionTest {
    ...
    void graphProjectionIsStructuralWithTypedEdges() {
        PipelineGraph g = new PipelineGraph("DEMO", true,
                List.of(PipelineNode.of("acq", "acquisition"),
                        ...
                        new PipelineNode("v", "sink.view", "Active subs", "last 24h",
                ...
                List.of(PipelineEdge.data("acq", "parse"),
                        PipelineEdge.data("parse", "v"),
                        new PipelineEdge("parse", PipelineRel.UNMATCHED, "quar"),
                        new PipelineEdge("parse", PipelineRel.route("emea"), "v")));

        Map<String, Object> proj = PipelineProjection.graph(g);
    ...
    void summaryOfLiftedLegacyPipeline(@TempDir Path dir) throws Exception {
        ...
        Map<String, Object> s = PipelineProjection.summary(PipelineLift.lift(cfg));
    ...
    void combinedJoinsPipelineAndJobAtTheSharedStore() {
        ...
        PipelineGraph producer = new PipelineGraph("ORDERS_ETL", true,
                List.of(PipelineNode.of("acq", "acquisition"),
                        new PipelineNode("sink", "sink.persistent", "Orders", null,
                                Map.of(PipelineStores.CONFIG_STORE, "orders"), null)),
        ...
        PipelineGraph consumer = new PipelineGraph("ORDERS_ROLLUP", true,
                ...
                List.of(PipelineEdge.data("src", "kpi")));

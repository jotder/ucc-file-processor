...
import {
    GammaNavigationItem,
    GammaVerticalNavigationAppearance,
    GammaVerticalNavigationMode,
    GammaVerticalNavigationPosition,
} from '@gamma/components/navigation/navigation.types';
import { GammaVerticalNavigationAsideItemComponent } from '@gamma/components/navigation/vertical/components/aside/aside.component';
...
import { GammaVerticalNavigationGroupItemComponent } from '@gamma/components/navigation/vertical/components/group/group.component';
...
import { GammaScrollbarDirective } from '@gamma/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'gamma-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: gammaAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'gammaVerticalNavigation',
    imports: [
        GammaScrollbarDirective,
        GammaVerticalNavigationAsideItemComponent,
        ...
        GammaVerticalNavigationGroupItemComponent,
        GammaVerticalNavigationSpacerItemComponent,
    ],
})
export class GammaVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: GammaVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: GammaVerticalNavigationMode = 'side';
    @Input() name: string = this._gammaUtilsService.randomId();
    @Input() navigation: GammaNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: GammaVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<GammaVerticalNavigationAppearance> =
        new EventEmitter<GammaVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<GammaVerticalNavigationMode> =
        new EventEmitter<GammaVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<GammaVerticalNavigationPosition> =
        new EventEmitter<GammaVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<GammaNavigationItem> =
        new ReplaySubject<GammaNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<GammaNavigationItem> =
        new ReplaySubject<GammaNavigationItem>(1);
    ...
    private _gammaScrollbarDirectives!: QueryList<GammaScrollbarDirective>;
    private _gammaScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'gamma-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`gamma-vertical-navigation-appearance-${this.appearance}`]: true,
            'gamma-vertical-navigation-hover': this._hovered,
            'gamma-vertical-navigation-inner': this.inner,
            'gamma-vertical-navigation-mode-over': this.mode === 'over',
            'gamma-vertical-navigation-mode-side': this.mode === 'side',
            'gamma-vertical-navigation-opened': this.opened,
            'gamma-vertical-navigation-position-left': this.position === 'left',
            'gamma-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for gammaScrollbarDirectives
     */
    @ViewChildren(GammaScrollbarDirective)
    set gammaScrollbarDirectives(
        gammaScrollbarDirectives: QueryList<GammaScrollbarDirective>
    ) {
        // Store the directives
        this._gammaScrollbarDirectives = gammaScrollbarDirectives;

        // Return if there are no directives
        if (gammaScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._gammaScrollbarDirectivesSubscription) {
            this._gammaScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._gammaScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                gammaScrollbarDirectives.forEach((gammaScrollbarDirective) => {
                    gammaScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.gamma-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._gammaScrollbarDirectives.forEach(
                    (gammaScrollbarDirective) => {
                        // Skip if not enabled
                        if (!gammaScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        gammaScrollbarDirective.scrollToElement(
                            '.gamma-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: GammaNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: GammaNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('gamma-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'gamma-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'gamma-vertical-navigation-aside-overlay'
        );
